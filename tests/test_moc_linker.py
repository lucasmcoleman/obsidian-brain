...
# ── should_skip now delegates to safe_paths.is_scannable_md, plus MOC_SUBDIR ────
@pytest.mark.parametrize("rel", [
    "_brain/entities/x.md",       # _brain at any depth, no entities carve-out
    "Projects/_brain/x.md",       # _brain matched at any depth, not just top-level
    ".trash/x.md",
    ".obsidian/x.md",
    "MOCs/x.md",
    "sub/MOCs/x.md",
    "livesync_log_foo.md",
    ".git/x.md",                  # delta 1: any dot-directory now skipped
])
def test_should_skip_true(vault, rel):
    assert ml.should_skip(vault / rel, vault) is True


def test_should_skip_false_for_ordinary_note(vault):
    assert ml.should_skip(vault / "Projects/note.md", vault) is False
