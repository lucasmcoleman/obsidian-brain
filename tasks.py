...
def _code_fence_mask(lines: list[str]) -> list[bool]:
    ...
    sample . A fence is only closed by the same delimiter
...
def _iter_md_files(vault: Path):
    # Skip non-knowledge paths via the shared predicate: any _brain/ path (no
    # entities carve-out here — see safe_paths.is_scannable_md), Obsidian's
    # .trash/ (deleted notes) and other dot-dirs (.obsidian/), plus LiveSync
    # debug logs. A .trash/ copy of a deleted note otherwise resurfaces its
...
def complete_task(
    ...
    # later force every line ending to LF on rewrite).
