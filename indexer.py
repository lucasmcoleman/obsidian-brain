...
def cleanup_tmp_files() -> None:
    ...
    never be mistaken for a real index ."""
...
def _build_lock():
    ...
    the container's scheduler build and corrupt the index .
...
def _index_params() -> dict[str, Any]:
    ...
    serving a stale-model / stale-chunking index ."""
    return {
...
def _index_is_consistent(existing_meta: dict[str, Any]) -> bool:
    ...
    zero results with no rebuild path ."""
    try:
...
def _vault_signature(notes: list[dict[str, Any]]) -> str:
    ...
    mtime, which can never decrease when a note is removed ."""
    h = hashlib.sha256()
...
# for topics it merely links to . `:end` (not bare `end`) anchors
...
def _is_excluded_path(rel_path: Path) -> bool:
    """True for markdown paths that look like notes but are NOT knowledge and must
    never be indexed. Delegates to safe_paths.is_scannable_md with indexer's
    policy: entity notes under _brain/entities/ stay indexable (M-D), and the
    _brain/ exclusion is anchored at the vault top level only.
    """
    return not is_scannable_md(rel_path, include_entities=True, brain_top_level_only=True)


def scan_vault(vault_path: str) -> list[dict[str, Any]]:
    """
    Walk the vault and return all markdown notes with their content.
    Skips non-knowledge paths (_brain/, .trash/, dot-dirs, livesync logs) — see
    _is_excluded_path.
    """
    notes = []
...
def build_index(force: bool = False) -> dict[str, Any]:
    ...
    if not force and os.path.exists(INDEX_PATH) and os.path.exists(METADATA_PATH):
        ...
        if existing_meta is not None:
            ...
            # renames, which a max-mtime check alone misses).
