...
```
   ...
   Pre-edit backups ─► MOC_BACKUP_DIR (OUTSIDE the vault, e.g. /server/.obsidian-moc-backups)
```

### Indexing data flow (`indexer.build_index`)

1. `ensure_dirs()` creates `_brain/` and `_brain/entities/`.
2. `scan_vault()` recursively globs `*.md`, **skips any path containing `_brain`**, strips leading YAML frontmatter (`content.split("---", 2)[2]`), and returns `{path, abs_path, content, mtime}` per note. Per-file read errors are silently skipped.
...
### Legacy prototype (do not use)

| File | Status |
|------|--------|
| `obsidian_brain.py` | **DEPRECATED.** The original self-contained, pure-stdlib prototype (JSON index in `vault_index.json`, brute-force cosine search). Superseded by the FAISS pipeline above; nothing in the repo imports it and it should not be extended. Its default embedding model (`text-embedding-nomic-embed-text-v1.5`) differs from production, so its index is **not** interchangeable. Retained for historical reference only. |
...
- **Python 3.11+**
- A running **LM Studio** (or any OpenAI-compatible) **embeddings** server reachable on the LAN — default `http://192.168.0.29:1234/v1`, serving `text-embedding-nomic-embed-text-v2-moe`. Embedding and querying both require this endpoint to be up.
...
> The maintenance scripts (`moc_linker.py`, `ledger_update.py`) and the legacy `obsidian_brain.py` use **only the Python standard library** — they run even without the pip dependencies installed.

### Install

```bash
cd /server/programming/obsidian-brain
pip install -r requirements.txt

# Build the initial index (requires the embeddings endpoint to be up):
OBSIDIAN_VAULT_PATH=/server/obsidian python3 indexer.py --force
```
...
|----------|---------|---------|
| `OBSIDIAN_VAULT_PATH` | `/server/obsidian` | Vault root. All derived paths (`_brain/`, `index.faiss`, `metadata.json`, `entities/`) are computed from it. The primary env var for the core path. Hard-required (exit 1) by `consolidate.py`. |
| *`LM_BASE_URL`* | `http://192.168.0.29:1234/v1` | **Hardcoded constant.** OpenAI-compatible embeddings base URL. Edit `config.py` to change. |
| *`EMBEDDING_MODEL`* | `text-embedding-nomic-embed-text-v2-moe` | **Hardcoded constant.** Embedding model id. Edit `config.py` to change. |
...
| Variable | Default | Meaning |
|----------|---------|---------|
| `TZ` | unset in code; compose sets `America/New_York` | Container local timezone so `BRAIN_REFRESH_AT_HOUR` resolves to the intended local time (`tzdata` is installed in the image). |
...
### Legacy `obsidian_brain.py` env vars (deprecated, listed for completeness)

| Variable | Default |
|----------|---------|
| `OBSIDIAN_VAULT_PATH` | `/server/obsidian` |
| `LMSTUDIO_URL` | `http://192.168.0.29:1234/v1` |
| `LMSTUDIO_EMBED_MODEL` | `text-embedding-nomic-embed-text-v1.5` *(differs from production model)* |
| `OBSIDIAN_CHUNK_CHARS` | `1200` |
| `OBSIDIAN_CHUNK_OVERLAP` | `200` |
...
python3 searcher.py what did Lucas decide about the Delta Heartland project
...
OBSIDIAN_VAULT_PATH=/server/obsidian python3 consolidate.py
...
python3 moc_linker.py --apply --endpoint http://192.168.0.29:1234/v1 --model gpt-oss-20b
...
**`moc_linker.py` flags:** `--vault` (default `/server/obsidian`), `--endpoint` (chat URL, default `http://localhost:4004/v1`), `--model` (default `unsloth/Qwen3.6-35B-A3B-MTP-GGUF`), `--timeout` (180s), `--retries` (3), `--exclude-dailies` (skip `Daily Notes/`, which is included by default), `--tag-notes`, `--related`, `--embed-endpoint` (default `http://192.168.0.29:1234/v1`), `--embed-model` (default `text-embedding-nomic-embed-text-v2-moe`), `--top-related` (5), and the mutually-exclusive `--dry-run` (default) / `--apply`. Exit codes: `0` success, `2` if the vault is missing or no target MOCs are discovered. Classification runs unless **only** `--related` was given; `--tag-notes` forces classification even alongside `--related`.
...
- `.dockerignore` excludes docs (`README.md`, `SKILL.md`, `CLAUDE.md`, `RESUME.md`, `obsidian-brain.md`), `__pycache__`, and `*.bak-premove`/`*.log`/`*.tar.gz`.

### Where it runs (from `deploy/README.md`)

- **Compose file:** `/server/docker/compose/mcp/docker-compose.yml`, service `obsidian-brain-mcp`.
- **Build context:** `/server/programming/obsidian-brain`.
- **Host port:** `8053` → container `8000`.
- **Endpoints:** `http://<host>:8053/mcp` (MCP), `http://<host>:8053/health` (liveness).
- **Network:** external `backend` network (shared with other MCP services; reaches LM Studio on the LAN).
- **Volumes:** `/server/obsidian:/vault` and `/server/.obsidian-moc-backups:/backups` (durable pre-edit backups, outside the vault).
- **Hardening:** `security_opt: [no-new-privileges:true]`; compose-level healthcheck on `/health`.

The service block sets `OBSIDIAN_VAULT_PATH=/vault`, `LM_BASE_URL=http://192.168.0.29:1234/v1`, `EMBEDDING_MODEL=text-embedding-nomic-embed-text-v2-moe`, `TZ=America/New_York`, `BRAIN_REFRESH_AT_HOUR=3`, `BRAIN_LINKER_ENABLED=1`, `BRAIN_LEDGER_ENABLED=1`, `LINKER_CHAT_URL=http://192.168.0.29:4004/v1`, `LINKER_CHAT_MODEL=unsloth/Qwen3.6-35B-A3B-MTP-GGUF`, `MOC_BACKUP_DIR=/backups`.
...
- **`_brain/` is excluded from scanning everywhere** (`scan_vault`, `tasks.py`, `moc_linker.scan_notes`), so the brain's own artifacts never feed back into the index or task lists.
...
Relevant source files (all under `/server/programming/obsidian-brain`): `config.py`, `embedder.py`, `indexer.py`, `searcher.py`, `brain.py`, `tasks.py`, `consolidate.py`, `mcp_server.py`, `moc_linker.py`, `ledger_update.py`, `obsidian_brain.py` (legacy), `Dockerfile`, `requirements.txt`, `.dockerignore`, `deploy/README.md`.
